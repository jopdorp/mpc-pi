...
| `0039` | MPC2000XL LCD frame export | Host-side only (opt-in) | Raw `MAME_MPC_LCD_EXPORT=<path>`; no launcher mode; default off | On each changed frame (0033 dirty detector) the 248x60 panel is rewritten to the export file as a 16-byte sequence-numbered header plus one byte per pixel, consumed by `scripts/maschine/mpc-mk1-display.py` for the Maschine MK1 display sink. Emulation path untouched; frozen PCM verified exact with the export active |
| `0040` | V53 wait-service HLEs (BRK 92 feed-flag, BRK 77 tick-read) | Exact fast path (opt-in) | `MPC_V53_FEED_FLAG_MODE=hle|accurate` and `MPC_V53_TICK_READ_MODE=hle|accurate`; raw `MAME_MPC_V53_BRK92_HLE` / `MAME_MPC_V53_BRK77_HLE`; launcher and raw MAME default `accurate`; fastest preset `hle` | BRK88-pattern substitution of the two hottest firmware wait services: the `INT 92` feed-flag peek/consume handler at `0000:0fe3` (3.09% of guest instruction dispatches, ~250k calls per render from the feed-wait loop) and the `INT 77` tick-read handler at `0000:169b` (0.65%). Byte-validated with word-aligned write taps (both handlers start on odd addresses), transient stack writes and register clobbers preserved exactly, per-path cycle charges empirically measured with the `MAME_MPC_V53_ISR23_MEASURE` diagnostic (peek 54, consume 65, tick-read 41, all excluding the caller's 24-cycle INT charge and all equal to the V33 table sums - this handler family has no prefetch stalls). Frozen PCM bit-identical with both HLEs active, and the measurement hook run against the HLE path reproduces the interpreted deltas and counts exactly (78x249,364 / 89x79 / 65x87,065). Deterministic instruction-count A/B on the official render: -0.86% host instructions; wall clock within noise on the desktop. The same diagnostic falsified the plan's `int 0x23` HLE premise: the hardware feed ISR fires only 42 times per render (delta 55, dead constant), so it is not the 31.4 kHz feed and not worth substituting |
| `0041` | uPD7810 timer event countdown | Exact fast path (opt-in, A53 candidate) | `MPC_PANEL_TIMER_COUNTDOWN=countdown|accurate`; raw `MAME_MPC_PANEL_TIMER_FAST`; default `accurate` everywhere, including the fast preset. Diagnostics: `MAME_MPC_PANEL_PC_HISTOGRAM=<path>`, `MAME_MPC_PANEL_IDLE_STATS=<hex pc>` | The panel uPD78C10 walks every timer, F/F, serial and ADC block on every instruction; during playback that walk is 6.8% of host time. This computes the distance to the next observable event (timer 0/1 match, ETM0/ETM1 match or ECNT wrap, coalesced-F/F conditions, the next SIO half-interval edge, the ADC sample and conversion boundaries) and reduces `handle_timers` to an accumulate-and-compare until that horizon is reached, replaying the accumulated chunk in one call at the boundary. Exact because `handle_timers` only ever runs at instruction boundaries and the counter arithmetic is pure accumulation, so the replay leaves every counter, IRR bit, output transition and ADC sample on the cycle the unbatched walk would have produced; any unbounded configuration (CI-gated ETIMER, non-coalesced F/F, a converter reconfiguration) yields a zero horizon and stays byte-for-byte on the old path. Counter reads and timer-configuration writes carry barriers at 58 opcode sites, enumerated by script and then verified by an independent audit of all 1,002 opcode bodies, which caught four indirect `SETB`/`CLR` writes the pattern list missed. Frozen PCM bit-identical off and on, with 17.7M fast-path hits against 5.65M materializes (75.8%). Deterministic instruction-count A/B: -2.51% host instructions with cycles flat (14.651e9 against 14.654e9) and desktop wall clock inside noise, so it is retained default-off as a Cortex-A53 candidate on the same basis as `0036` and `0038` |
| `0042` | MPC2000XL panel event-bus HLE | Latency-changing (opt-in) | `MPC_PANEL_MODE=hle` alongside `accurate`/`event`; raw `MAME_MPC_PANEL_HLE`; default `accurate`, fastest preset `event`, turbo preset `hle`. Diagnostics: `MAME_MPC_PANEL_TX_LOG`, `MAME_MPC_PANEL_SLIDER_SWEEP` | Suspends the uPD78C10 front-panel CPU and synthesizes its serial stream instead. The panel spends about ten thousand interpreted instructions per byte it transmits (35.7M instructions for 3,586 bytes in a 60 s render) scanning a matrix that does not exist when the panel is external hardware, and its link to the V53 is one way, so the whole device reduces to a two-byte message generator: boot handshake, the `0x86` slider/keep-alive report, key edges from a measured 46-code table bound by field name, and encoder detents. See [`mpc2000xl-panel-protocol.md`](mpc2000xl-panel-protocol.md). Not accuracy-preserving by construction, so it is gated functionally rather than by the frozen PCM: the machine boots to a correct screen (LCD export matches the reference at 4,600 lit pixels) and a full project render matches the reference audio to `rms` 883.5 against 883.5 with peaks within 0.1%. Deterministic instruction and cycle counts measure -15.8% instructions and -23.2% cycles against the panel running, with the panel CPU verified down to 1,975 executed instructions from 35,721,180. Wall-clock runs read around 2200% against a 1279% baseline, but that harness varies by more than 20% run to run and is not the basis for the claim. Also routes MIDI notes 52-97 to panel keys so an external controller's buttons reach the firmware on the panel link exactly as its pads already did |
| `0043` | MIDI poll rate and DSP DMA pacing | Latency-changing (opt-in) | Raw `MAME_MPC_MIDI_POLL_HZ=<1500-96000>` and `MAME_MPC_DSP_DMA_TURBO=<1-64>`; both unset by default, turbo preset uses `8000` and `4` | Two host-visible latency sources outside the emulated CPUs. MAME polls MIDI input at a fixed 1500 Hz, adding up to 0.67 ms of queueing before a note reaches the machine at all; the option raises that rate, each extra poll being one non-blocking driver check. The L7A1045 paces DMA at one word per 64 DSP clocks, which is bus-bandwidth shaping on real hardware but in emulation only makes the firmware wait - it polls DMA-ready for 7.7% of its instruction dispatches - so the divider is configurable and the turbo preset runs it 16x faster. Deliberately not accuracy-preserving; a full project render with the DMA divider at 4 matched the reference audio at rms 883.5 against 883.5 with peaks within 0.01%. The DMA divider does not shorten input-to-sound latency at all - playback reads wave RAM directly and only loading and sampling use the DMA channel - but it does cut bulk transfer time by the same factor, roughly 83 ms to 5 ms per second of 44.1 kHz audio loaded. It is also a throughput win rather than a cost, because the firmware stops polling DMA-ready sooner than the extra timer callbacks cost. The curve is not monotonic, so the divider was swept: `8` measured best over two reproducible runs each (spread within 0.03%), with `4` 1.05% worse, `16` 1.65% worse, and `1` regressing past stock entirely. The turbo preset uses `8`; `1` should not be used despite being accepted. Host-side contributors are documented separately in [`host-latency-tuning.md`](host-latency-tuning.md) |
| `0044` | Guard PipeWire stream format positions against unfilled node data | Correctness | Always active | A node's port count and its channel-position array arrive on different PipeWire events, and `stream_sink_open`/`stream_source_open` indexed the position vector by the port count with no bound. Caught between the two events the vector is empty, `data()` of an empty vector is null, and the emulator segfaulted at startup - reproduced on the appliance, timing-dependent enough that whether stdout was a pipe decided whether it crashed. Missing entries fall back to `SPA_AUDIO_CHANNEL_UNKNOWN`; the loop is clamped to `SPA_AUDIO_MAX_CHANNELS`. Upstream-relevant |
| `0045` | Declare PipeWire stream latency and bound the pacing wait | Infrastructure | `MAME_PIPEWIRE_NODE_LATENCY` defaults to `1024` frames; diagnostics/configuration include `MAME_PIPEWIRE_BUFFERS`, `MAME_PIPEWIRE_REARM_AFTER`, `MAME_PIPEWIRE_PREBUFFER_QUANTA`, `MAME_PIPEWIRE_PACE_BUDGET_US`, and `MAME_PIPEWIRE_LATE_US` | A declared `1024`-frame node latency replaces the inherited one-quantum stream latency; the pacing wait is bounded and related callback/prebuffer faults are instrumented |
| `0046` | Raise the audio-producing effects thread above emulation under realtime scheduling | Infrastructure | `MAME_SOUND_EFFECTS_PRIORITY`; unset or `0` retains prior behavior, otherwise defaults to creator priority plus `40`, clamped to the policy maximum | On the Raspberry Pi 5 at a 64-frame quantum, output rose from 9.7% to 99.8% of realtime with zero underruns/overruns |
| `0047` | Make MIDI panel-key injection atomic | Correctness | Always active | A two-byte panel event is enqueued as a pair, avoiding lasting command/keycode stream desynchronisation when the queue is full |
| `0048` | Enlarge the panel injection queue and report whole-message drops | Correctness | Always active | Uses a 512-byte queue with 16-bit indices; its maximum backlog is 2 ms at the stated 4 us byte-drain rate |
| `0049` | Reach the DATA wheel and NOTE VARIATION slider over MIDI control change | Functionality | Always active | CC 1 maps relative two's-complement wheel steps and CC 2 maps the absolute slider; large wheel deltas emit repeated steps |
| `0050` | Let a PipeWire prebuffer cushion actually be one quantum | Latency-changing infrastructure | `MAME_PIPEWIRE_PREBUFFER_QUANTA=1` selects one quantum | Removes the redundant producer-frame floor, so one requested quantum is 1.45 ms rather than 2.90 ms at a 64-frame, 44.1 kHz quantum |
| `0051` | Give the L7A1045 audio inputs and wire the ADC to DMA channel 2 | Functionality | Always active; recording is gated by the machine driver from WADCSN, the FPGA register the firmware strobes; diagnostic `MAME_MPC_SAMPLE_DEBUG` | The device had zero input streams, so the emulated MPC could not record at all. Disassembly of the v1.20 sampling module puts the ADC on DMA channel 2 (mode `0x55`, auto-init, 16-bit, I/O-to-memory, into a 2 KiB ring at linear `0x7F000`), with channel 3 then pushing blocks of that buffer into wave RAM - the first attempt served capture from channel 3's read callback and could only corrupt sample loading, because channel 3 runs in the write direction during sampling. Captured words are interleaved one per enabled channel, L before R, exactly as the meter routine reads them, and the ring discards the oldest audio rather than growing without bound. See [`mpc2000xl-sampling-protocol.md`](mpc2000xl-sampling-protocol.md) |
| `0052` | Drop the `params` declaration `0045` orphaned | Correctness | Always active | `0045` replaced this declaration's only use with `connect_params[]` and left the declaration behind - harmless on the desktop toolchain, fatal on the Buildroot aarch64 cross build, which compiles with `-Werror` and is the build that ships. The identical declaration further down is genuinely used; only this one is orphaned |
| `0053` | Open the sampling input at machine start | Host-side only | Always active | While idle, reads and discards both input channels so the OSD capture node exists before the firmware arms recording. Disproven by the cold-start measurement behind `0057` - a binary carrying this read shows the same empty graph, because the read never created anything - and the read is removed again by `0055` |
| `0054` | Temporary sampling diagnostics: take dump and idle-input census | Diagnostics (temporary) | `MAME_MPC_SAMPLE_DUMP=<prefix>` arms the take dump; key-on register logging and the idle peak/count census run under `MAME_MPC_SAMPLE_DEBUG` | Marked `TEMPORARY DIAGNOSTICS ... not for release` in its own comments; answers cheaply whether captured audio gets as far as the ADC ring and whether the guest comes and takes it |
| `0055` | Start stereo pairs on one key-on and do not play voices being recorded | Correctness | Always active | Replaces the previous `0055` wholesale; the header records a measured stereo sampling path using the sample rig and an exported LCD confirmation |
| `0056` | Keep the wave transfer behind the ADC | Correctness | Always active | Gates channel 3 until channel 2 is ahead, preventing stale capture-ring words and the resulting block-rate buzz |
| `0057` | Keep saved sound mappings for host nodes that are temporarily absent | Correctness | Always active | An unresolved mapping is retained for a later graph generation instead of being deleted, allowing the sampling input node to appear after its launcher-created sink |
| `0058` | Clock ADC DMA channel 2 one word per conversion | Correctness | Always active | An ADC timer makes a converted word due per enabled channel/sample period and only asserts DRQ when that word and ring data exist, keeping the level meter's address register moving |
